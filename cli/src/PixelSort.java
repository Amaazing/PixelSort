...
public class PixelSort extends PApplet{
	...
	public Column_sorter cs;

	public PImage img, img_orig;
	String img_file = "img3.jpg";
	String img_url = java.nio.file.Paths.get("").toAbsolutePath().toString()+"\\src\\"+img_file;
	...
	public void sort_column(int column_number) {
		...
		for (int pixel_pointer=column_start, row=0; pixel_pointer < column_end; pixel_pointer+=(width), row++) {
			int pixel = img.pixels[pixel_pointer];
			...
			if (sub_end > -1) {
				...
				for ( int sub_pointer = sub_start, count = 0; count < rows_count; sub_pointer += width, count ++ ){
					...
					sub_array[count] = img.pixels[sub_pointer];
				}
				sub_array = sort(sub_array);
				sub_array = reverse(sub_array);

				// put subarray pixels into img.pixels[], original index stored in map
				for( int map_pointer = 0; map_pointer < map_array.length; map_pointer ++ )
					img.pixels[map_array[map_pointer]] = sub_array[map_pointer];

				///////// MARKERS
				//img.pixels[sub_start] = color(0xff0000);
				//img.pixels[sub_end] = color(0x00ff00);
				/////////				
